...
>(({ children, className, hideBorder, isHeader, style, ...props }, ref) => {
  ...
  return (
    <PlateElement
      ...
      as={isHeader ? 'th' : 'td'}
      className={cn(
        'relative h-full overflow-visible border-none bg-white p-0 dark:bg-neutral-950',
        ...
        !hideBorder &&
          cn(
            ...
            'before:size-full',
            selected && 'before:z-10 before:bg-neutral-100 dark:before:bg-neutral-800',
          ...
          {!readOnly && (
            <>
              ...
              {hovered && (
                <div
                  className={cn(
                    'absolute -top-3 z-30 h-[calc(100%_+_12px)] w-1 bg-neutral-950 dark:bg-neutral-300',
                    'right-[-1.5px]'
                  )}
                />
              )}
